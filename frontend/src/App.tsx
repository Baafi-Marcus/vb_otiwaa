...
function AuthPage({ desiredRole, forcedUsername }: { desiredRole: UserRole, forcedUsername?: string }) {
  ...
  // 3. Login Screen
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#020202] via-[#0a0a0a] to-[#020202] text-white flex flex-col items-center justify-center p-4 sm:p-6 relative overflow-y-auto">
      <BackgroundEffects />
      ...
      <motion.div
        ...
        className="w-full max-w-md relative z-10 py-20 sm:py-24"
      >
        {/* Glowing Card Effect */}
        <div className="absolute inset-0 bg-gradient-to-r from-blue-500/20 via-purple-500/20 to-pink-500/20 rounded-[2rem] blur-3xl opacity-50" />

        <div className="relative bg-black/40 backdrop-blur-2xl border border-white/10 p-6 sm:p-10 rounded-[2rem] shadow-2xl">
          {/* Header */}
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="relative group">
                <div className="absolute inset-0 bg-gradient-to-r from-blue-500/20 to-purple-500/20 rounded-2xl blur opacity-0 group-focus-within:opacity-100 transition-opacity" />
                <Lock className="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground group-focus-within:text-white transition-colors z-10" />
                <input
                  type="password"
                  required
                  value={password}
                  ...
                  className="relative w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-11 sm:pl-12 pr-4 text-sm font-medium focus:outline-none focus:border-white/30 focus:bg-white/10 transition-all placeholder:text-white/30"
                />
              </div>
            </div>
